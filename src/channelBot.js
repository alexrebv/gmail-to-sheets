...
async function sendPriceListReport(chatId, threadId, arg, cfg) {
  ...
  try {
    ...
    // Источников два - база приложения и таблица, - и первое, что надо
    // сказать: откуда прайс взялся на самом деле.
    const lines = ['ПРАЙС ДЛЯ ПОЛНОГО БЛАНКА', `Источник: ${pricelist.sourceText(d)}`];
    if (d.source === 'база') {
      lines.push(d.dbOwn ? 'База задана отдельно (UCHET_DATABASE_URL)' : 'База - та же, что у сервиса');
    } else {
      // До таблицы дошли либо потому, что так велели, либо потому, что база
      // не ответила: причину надо назвать, иначе её не найти.
      if (d.want === 'таблица') lines.push('Так задано настройкой PRICELIST_SOURCE');
      else if (d.dbNote) lines.push(`База не сработала: ${d.dbNote}`);
      else if (!d.dbUrl) lines.push('База не подключена: нет ни UCHET_DATABASE_URL, ни DATABASE_URL');
      lines.push(tryId ? `Проверяю таблицу из команды: ${tryId.slice(0, 12)}…`
        : `Таблица: ${d.spreadsheetId ? d.spreadsheetId.slice(0, 12) + '…' : 'не задана'}${d.ownId ? ' (основная)' : ' (задана отдельно)'}`);
      lines.push(`Лист: ${d.usedSheet || d.sheet}${d.usedSheet && d.usedSheet !== d.sheet ? ` (искали «${d.sheet}», взяли похожий)` : ''}`);
    }
    lines.push(`Строк в прайсе: ${d.rows}`);
    // В настройках пишут кусок названия - так задумано, поэтому и говорим
    // «кто содержит», а не «кто равен».
    lines.push(`Полный бланк положен тем, в чьём названии есть: ${wanted.join(', ') || '—'}`);
